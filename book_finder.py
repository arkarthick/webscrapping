...
def open(url):
    #this function will open the link , read the file and return the file as object
    opened = False
    while not opened:
        try:
            page = urlopen(url)
...
def grab_data(input_data):
    result_size = 100 # 25 or 50 or 100
    url = f'http://gen.lib.rus.ec/search.php?req={input_data}&open=0&res={result_size}&view=simple&phrase=1&column=def'
    html = open(url)

    pg_soup = bs(html, 'html.parser')
    table = pg_soup.findAll('tr',{'valign':'top'})
    table = table[1:] #grab all table containes book
    ...
    for book_data in books_data:
        ...
        # pg_link = book_data[9].a['href'] #pass the linnk to the variable
        # html = open(pg_link) #open the link
        # pg_soup = bs(html, 'html.parser')
        # link_data = pg_soup.findAll('td',{'id':'info'})
        # file_link = link_data[0].h2.a['href']  #find the direct downloadable link
        # link = ip + file_link
